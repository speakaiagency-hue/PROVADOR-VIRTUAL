...
const urlToFile = (url: string, filename: string): Promise<File> => {
    return new Promise((resolve, reject) => {
        ...
        image.onload = () => {
            ...
            canvas.toBlob((blob) => {
                if (!blob) {
                    return reject(new Error('Falha ao converter canvas em blob.'));
                }
        ...
        image.onerror = (error) => {
            reject(new Error(`Não foi possível carregar a imagem da URL para conversão. Erro: ${error}`));
        };
...
const WardrobePanel: React.FC<WardrobePanelProps> = ({ onGarmentSelect, activeGarmentIds, isLoading, wardrobe }) => {
    ...
    const handleGarmentClick = async (item: WardrobeItem) => {
        ...
        try {
            ...
            const detailedError = `Falha ao carregar item do guarda-roupa. Geralmente é um problema de CORS. Verifique o console do desenvolvedor.`;
